...
ENV container=oci \
    MANPATH=/opt/rh/go-toolset-1.16/root/usr/share/man: \
    X_SCLS=go-toolset-1.16  \
    GOFLAGS='-mod=vendor' \
...
RUN yum install -y --setopt=skip_missing_names_on_install=False \
    ...
    'go-toolset-1.16*' goversioninfo openssl openssl-devel systemd-devel gpgme-devel libassuan-devel
RUN yum clean all
